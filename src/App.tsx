  ...
  return (
    <section className="hero">
      <motion.div className="hero__media" style={reduced ? undefined : { y: imageY }}>
        <motion.img
          src="/images/sparkclean-hero-concept.png"
      ...
      <div className="room-list shell">
        {roomChapters.map((room, index) => (
          <article className={`room-chapter ${index % 2 ? "room-chapter--reverse" : ""}`} key={room.id}>
            <Reveal className="room-chapter__image"><img src={room.image} alt={room.alt} loading={index > 0 ? "lazy" : "eager"} /><span>0{index + 1}</span></Reveal>
...
function InnerHero({ eyebrow, title, intro, image, alt }: { eyebrow: string; title: string; intro: string; image: string; alt: string }) {
  return <section className="inner-hero"><div className="shell inner-hero__grid"><Reveal><span className="eyebrow">{eyebrow}</span><h1>{title}</h1><p>{intro}</p><ParticleButton to="/estimate">Get your free estimate</ParticleButton></Reveal><Reveal className="inner-hero__image" delay={0.08}><img src={image} alt={alt} /></Reveal></div></section>;
}
